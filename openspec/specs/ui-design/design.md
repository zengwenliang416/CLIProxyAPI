---
version: 1.0.0
name: CLIProxyAPI Non-UI Contract
description: Backend-only project with no browser UI or design-system runtime.
colors:
  primary: "#171717"
  secondary: "#4d4d4d"
  tertiary: "#006bff"
...
# CLIProxyAPI Non-UI Contract

## Overview

CLIProxyAPI is a backend proxy and has no project-owned browser interface.
These tokens satisfy the shared SpecNav contract but are not a production UI
dependency. The terminal UI follows its existing Bubble Tea conventions.
...
- Theme capability: `none` for the backend API.
- Theme toggle: `none`.
- Internationalization: no UI i18n runtime.
- Supported locales: none for UI; API error language follows existing route conventions.
- Default locale: none.
- Prototype rule: no UI prototype is required for backend-only changes.
